...
function GetCoverageListHsPackage() {
  ...
  // อ่านข้อมูลจากชีท
  const dataManager = FinAppsLibrary.newDataManager(sheetInfo)
...
function AddDataPlan(_formObj, _sheetInfo, _folderID) {
  ...
  // อ่านข้อมูลจากชีท
  const dataManager = FinAppsLibrary.newDataManager(_sheetInfo)
  ...
  if (dataExist.length === 0) { // ไม่พบข้อมูล เป็นข้อมูลใหม่
    ...
    if (dataForm.brochure.length > 0) { // มีการแนบไฟล์
      ...
      imageID = FinAppsLibrary.UploadFile(_folderID, dataForm.brochure, newDataID) // อัปโหลดไฟล์ และ รับข้อมูล imageID
      url = FinAppsLibrary.GetImageUrl(imageID).url // ข้อมูล lh5
    }
    ...
    //   lh5 = FinAppsLibrary.GetImageUrl(imageID).lh5 // รับข้อมูล lh5 ของไฟล์โดยปริยาย
    // }
...
function EditDataPlan(_formObj, _sheetInfo, _folderID) {
  ...
  if (existLength === 0) { // ไม่พบข้อมูลซ้ำ
    ...
    if (dataForm.brochure.length > 0) { // มีการแนบไฟล์ใหม่
      ...
      if (imageID !== '-') {
        const deleteFile = FinAppsLibrary.DeleteFile(imageID) // ลบไฟล์เก่า
      }
      imageID = FinAppsLibrary.UploadFile(_folderID, dataForm.brochure, dataForm.dataID) // ข้อมูลใหม่ BruchureID
      lh5 = FinAppsLibrary.GetImageUrl(imageID).lh5 // ข้อมูลใหม่ BrochureUrl
    }
...
function TemplateExistData(_sheetName, _dataForm, _Criteria) {
  ...
  let result = FinAppsLibrary.ResultInit(true) // ผลลัพธ์
  result.state = true
...
function AddDataHsPackage(_formObj) {
  ...
  // อ่านข้อมูลจากชีท
  const dataManager = FinAppsLibrary.newDataManager(sheetPackagesInfo)
  const dataExist = dataManager.getDataExistOr(criteriaObj) // อ่านข้อมูลจากชีท
  ...
  if (dataExist.length === 0) { // ไม่พบข้อมูล เป็นข้อมูลใหม่
    ...
    if (createPackage.state) { // สร้างแพ็คเกจสำเร็จ
      ...
      // อ่านข้อมูลจากชีท
      const dataManagerCoverage = FinAppsLibrary.newDataManager(sheetCoverageInfo)
...
function EditDataHsPackage(_formObj) {
  const result = FinAppsLibrary.ResultInit(false) // ผลลัพธ์
...
function SearchDataHsPackage(_criteria) {
  ...
  // ข้อมูลบริษัทประกัน
  const insurerDataManager = FinAppsLibrary.newDataManager(GetSheetInfo(SHEET_INSURERS))
  ...
  if ((insurerCriteriaObj.length > 0) && (packageCriteriaObj.length === 0)) { // ค้นหาแพ็คเกจทั้งหมดของบริษัท
    ...
    const planDataManager = FinAppsLibrary.newDataManager(GetSheetInfo(SHEET_HS_PLANS))
    ...
    const packageDataManager = FinAppsLibrary.newDataManager(GetSheetInfo(SHEET_HS_PACKAGES))
