...
function CreateData(_sheetInfo, _item) {
  ...
  const item = FinAppsLibrary.ReplaceEmptyToSymbol(headers, _item, '-') // แทนที่ '' ด้วย '-'
...
function CreateDataMultiRows(_sheetInfo, _item) {
  ...
  let items = []
  _item.forEach(data => {
    let temp = FinAppsLibrary.ReplaceEmptyToSymbol(headers, data, '-') // แทนที่ '' ด้วย '-'
    items.push(temp)
  })
...
function DeleteData(_jsonData) {
  ...
  // อ่านข้อมูลจากชีท
  const dataManeger = FinAppsLibrary.newDataManager(sheetInfo)
  ...
  if (readData.length > 0) { // พบข้อมูล
    ...
    if ((fileID !== null) && (fileID !== '-')) {
      FinAppsLibrary.DeleteFile(fileID) // ลบไฟล์
    }
