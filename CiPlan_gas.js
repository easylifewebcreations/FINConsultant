function AddDataRiderContract(_formObj, _sheetInfo, _folderID) {
  // Set result variable
  let result = FinAppsLibrary.ResultInit(false)
  // Spreadsheet
  ...
  const existCriteria = FinAppsLibrary.ExistCriteriaOR(spreadsheetID, sheetName, criteriaObj, keyHeader)
  if (!existCriteria) {
    // ******************** New item ********************
    // Get next data-id
    const headers = GetHeaders(sheetName)
    const newDataID = FinAppsLibrary.GetNextIDWithFormat(spreadsheetID, sheetName, headers, keyHeader, RIDERCONTRACT_ID_SETTING)
    // Get & upload file
    ...
    if (dataForm.brochure.length > 0) {
      ...
      imageID = FinAppsLibrary.UploadFile(_folderID, dataForm.brochure, newDataID)
      urls = FinAppsLibrary.GetImageUrl(imageID)
      lh5 = urls.lh5
    } else {
...
function EditDataRiderContract(_formObj, _sheetInfo, _folderID) {
  ...
  const existCriteriaOR = FinAppsLibrary.ExistCriteriaOR(spreadsheetID, sheetName, criteriaObj, keyHeader)
  // let existItem = {
  ...
  if (!existCriteria) {
    ...
    // ]
    // const existItem = FinAppsLibrary.ReadItem(spreadsheetID, sheetName, criteriaObj)
    ...
    const readItem = FinAppsLibrary.ReadItem(spreadsheetID, sheetName, criteriaOldItemObj)
    ...
    if (dataForm.brochure.length > 0) {
      // ******************** File uploaded ********************
      // Delete file
      const deleteFile = FinAppsLibrary.DeleteFile(imageID)
      // Upload & get file info
      imageID = FinAppsLibrary.UploadFile(_folderID, dataForm.brochure, dataForm.dataID)
      const urls = FinAppsLibrary.GetImageUrl(imageID)
      lh5 = urls.lh5
    }
