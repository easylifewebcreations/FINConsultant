function AddDataCustomer(_formObj, _sheetInfo, _folderID) {
  ...
  if (dataExist.length === 0) { // ไม่พบข้อมูล เป็นข้อมูลใหม่
    ...
    if (dataForm.profilePicture.length > 0) { // มีการแนบไฟล์
      ...
      imageID = FinAppsLibrary.UploadFile(_folderID, dataForm.profilePicture, newDataID) // อัปโหลดไฟล์ และ รับข้อมูล imageID
      lh5 = FinAppsLibrary.GetImageUrl(imageID).lh5 // ข้อมูล lh5
...
function GetDataFormCustomer(_formObj) {
  return {
    ...
    mobile: FinAppsLibrary.CheckPhoneNumber(_formObj.mobile.trim()),
