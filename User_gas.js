...
function AddDataUser(_formObj, _sheetInfo, _folderID) {
  ...
  if (dataExist.length === 0) { // ไม่พบข้อมูล เป็นข้อมูลใหม่
    ...
    if (dataForm.profilePicture.length > 0) { // มีการแนบไฟล์
      ...
      imageID = FinAppsLibrary.UploadFile(_folderID, dataForm.profilePicture, newDataID) // imageID
      lh5 = FinAppsLibrary.GetImageUrl(imageID).lh5 // lh5
      ...
      lh5 = FinAppsLibrary.GetImageUrl(imageID).lh5 // ข้อมูล lh5 โดยปริยาย
    }
    ...
    const item = {
      ...
      Password: FinAppsLibrary.EncodeSignin(dataForm.username, DEFAULT_PASSWORD),
...
function EditDataUser(_formObj, _sheetInfo, _folderID) {
  ...
  if (readData.length > 0) { // มีข้อมูล
    ...
    if (dataForm.profilePicture.length > 0) { // มีการแนบไฟล์ใหม่
      ...
      if (imageID !== '-') {
        const deleteFile = FinAppsLibrary.DeleteFile(imageID) // ลบโปรไฟล์เก่า
      }

      // อัปโหลดไฟล์ และ เก็บ url
      imageID = FinAppsLibrary.UploadFile(_folderID, dataForm.profilePicture, dataForm.dataID) // ข้อมูลใหม่ imageID
      lh5 = FinAppsLibrary.GetImageUrl(imageID).lh5 // ข้อมูลใหม่ lh5
    }
...
function ResetPassword(_jsonData) {
  ...
  if (readData.length > 0) { // มีข้อมูล
    ...
    item.Password = FinAppsLibrary.EncodeSignin(item.Username, DEFAULT_PASSWORD)
    item.ForceReset = YES
